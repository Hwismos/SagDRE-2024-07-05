...
class BiLSTM(nn.Module):
    ...
    def forward(self, src, src_lengths):
        ...
        packed_src = nn.utils.rnn.pack_padded_sequence(
            new_src, new_src_lengths.cpu(), batch_first=True, enforce_sorted=True
        )
        packed_outputs, (src_h_t, src_c_t) = self.lstm(packed_src)

        outputs, _ = nn.utils.rnn.pad_packed_sequence(
            packed_outputs, batch_first=True, padding_value=self.config.word_pad
        )
...
class RelEdgeLayer(nn.Module):
    def __init__(self, node_feat, edge_feat, activation, dropout=0.0):
