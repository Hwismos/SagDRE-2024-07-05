...
def get_opt():
    ...
    # datasets path
    parser.add_argument(
        "--train_set", type=str, default=os.path.join(data_dir, "train_annotated.json")
    )
    parser.add_argument(
        "--dev_set", type=str, default=os.path.join(data_dir, "dev.json")
    )
    parser.add_argument(
        "--test_set", type=str, default=os.path.join(data_dir, "test.json")
    )
    ...
    # use GLOVE / BERT encoder, default: BERT encoder
    parser.add_argument(
    ...
    data_opt.rel2id = json.load(
        open(os.path.join(data_opt.data_dir, "rel2id.json"), "r")
    )
    data_opt.id2rel = {v: k for k, v in data_opt.rel2id.items()}
    data_opt.word2id = json.load(
        open(os.path.join(data_opt.data_dir, "word2id.json"), "r")
    )
    data_opt.ner2id = json.load(
        open(os.path.join(data_opt.data_dir, "ner2id.json"), "r")
    )
