...
def train(opt, data_opt):
    ...
    if opt.use_model == "bert":
        ...
        base_params = filter(
            lambda p: p.requires_grad and id(p) not in bert_param_ids,
            model.parameters(),
        )
    ...
    for epoch in range(start_epoch, opt.epoch + 1):
        ...
        for d in tqdm(train_loader, desc="Epoch " + str(epoch), unit="b"):
            ...
            loss2 = Aloss(m_preds.view(-1, 97), m_label.view(-1, 97), r_mask.view(-1))
            loss1 = torch.sum(BCE(m_preds, m_label) * r_mask.unsqueeze(2)) / (
        ...
        log_str = (
            "| epoch {:2d} | loss_re {:5.3f} | loss_re2 {:5.3f} | "
            + "NA acc: {:4.2f} | noNA acc: {:4.2f}  | tot acc: {:4.2f} "
        )
        logging(
            log_str.format(
                epoch,
                re_loss * 100 / batch_num,
                re_loss2 / batch_num,
                acc_NA.get(),
                acc_not_NA.get(),
                acc_total.get(),
        ...
        if epoch > 25 or epoch % opt.test_epoch == 0:
            ...
            model.eval()
            ign_f1, _, _ = test(model, dev_loader, id2rel=data_opt.id2rel)
            if not opt.coslr:
                scheduler.step(ign_f1)
            model.train()
            logging(
                "end testing | epoch {:3d} | time: {:5.2f}s".format(
        ...
        if epoch % opt.save_model_freq == 0:
            ...
            torch.save(
                {"epoch": epoch, "lr": lr, "checkpoint": model.state_dict()}, path
            )
