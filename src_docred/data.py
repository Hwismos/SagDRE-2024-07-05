...
class GloveDGLREDataset(IterableDataset):
    ...
    def __init__(
        self, src_file, word2id, ner2id, rel2id, dataset="train", instance_in_train=None
    ):
    ...
    def process_doc(self, doc, dataset, word2id, ner2id):
        ...
        # ======================================================
        # compute subword to word index
        ent2word = np.zeros((len(entity_list), self.document_max_length))
        # compute convertion matrix for entity
        for idx, poss in ent_idx.items():
    ...
    def __getitem__(self, idx):
        doc = self.data[idx]
        cur_d = self.process_doc(
            doc, dataset=self.dataset, word2id=self.word2id, ner2id=self.ner2id
        )
        return cur_d
